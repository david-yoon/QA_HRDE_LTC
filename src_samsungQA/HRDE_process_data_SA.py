...
DATA_DIR = '../data/samsungQA/'

DATA_FILE_NAME = 'SA_data.pkl'
TEST_DATA_FILE_NAME = 'SA_data_test.pkl'  # contains only 500 samples

VOCA_FILE_NAME = 'SA_dic.pkl'
GLOVE_FILE_NAME = 'SA_glove.pkl'
...
class ProcessData:
    ...
    # target_ids : 'r' (answer) -> split by using '__eos__' -> multiple sentneces, 
    ...
    def create_train_set(self):
        ...
        for index in xrange(data_len):
            ...
            turn =[x.strip() for x in (' '.join(str(e) for e in self.train_data['r'][index])).split(str(self.voca['__eos__']))]
    ...
    # source_ids : 'c' (question) 
    # list_target_ids : 'r' (answer) -> split by using '__eos__' -> multiple sentneces  --> 10 list ( true, false.... false )
    def create_valid_set(self):
        ...
        for index in xrange(data_len / 10):
            ...
            for i in xrange(10) :
                ...
                turn =[x.strip() for x in (' '.join(str(e) for e in self.valid_data['r'][t_index+i])).split(str(self.voca['__eos__']))]
                turn = [ x for x in turn if len(x) >1]
                tmp_ids = [x.split(' ') for x in turn]
                ...
                list_target_ids.append( target_ids )
    ...
    def create_test_set(self):
        ...
        for index in xrange(data_len / 10):
            ...
            for i in xrange(10) :
                ...
                turn =[x.strip() for x in (' '.join(str(e) for e in self.test_data['r'][t_index+i])).split(str(self.voca['__eos__']))]
                turn = [ x for x in turn if len(x) >1]
    ...
    def get_batch(self, data, batch_size, encoder_size, context_size, encoderR_size, is_test, start_index=0, target_index=1):
        ...
        for _ in xrange(batch_size):
            ...
            if is_test is False:
                ...
                list_encoder_input = data[index][1][target_index]
