...
from DE_process_data_v1 import *
...
class DualEncoderModel:
    ...
    def _create_embedding(self):
        ...
        with tf.name_scope('embed_layer'):
            ...
            """
            self.embed_matrixR = tf.Variable(tf.random_normal([self.source_vocab_size, self.embed_size],
                                                            mean=0.0,
                                                            stddev=0.01,
                                                            dtype=tf.float32,                                                             
                                                            seed=None),
            ...
            self.embed_enR      = tf.nn.embedding_lookup(self.embed_matrixR, self.encoderR_inputs, name='embed_encoderR')
            """
    ...
    def _create_gru_model(self):
        ...
        with tf.name_scope('siaseme_RNN') as scope:
            ...
            with tf.variable_scope("siaseme_GRU", reuse=True, initializer=tf.orthogonal_initializer()):      
                ...
                (outputs_enR, self.last_states_enR) = tf.nn.dynamic_rnn(
                                                    cell=cells_enR,
                ...
                self.final_encoderR  = self.last_states_enR[-1]
    ...
    def _add_memory_network(self):
        ...
        with tf.name_scope('memory_network_v1') as scope:
            ...
            self.memory = tf.get_variable( "latent_topic_memory", 
                                      shape=[self.topic_size, self.memory_dim],
            ...
            self.topic_sim_project = tf.matmul( self.final_encoder, self.memory_W ) + memory_bias
            
            topic_sim = tf.matmul( self.topic_sim_project, self.memory, transpose_b=True )
            ...
            self.rsum = tf.reshape( tmpT2, [self.batch_size, self.memory_dim])
            ...
            self.final_encoder_dimension  = self.hidden_dim + self.memory_dim   
            self.final_encoderR_dimension = self.hidden_dim
    ...
    def _create_summary(self):
        ...
        with tf.name_scope('summary'):
            ...
            tf.summary.histogram('batch_prob', self.batch_prob)
            
            tf.summary.histogram('memory', self.memory)
                        
            tf.summary.histogram('topic_similarity', self.topic_sim_sigmoid_softmax)
...
def main(batch_size, encoder_size, encoderR_size, num_layer, hidden_dim, embed_size,
         num_train_steps, lr, valid_freq, is_save, graph_dir_name, is_test, use_glove,
         dr, memory_dr,
         memory_dim, topic_size):
...
if __name__ == '__main__':
    ...
    p.add_argument('--memory_dr', type=float, default=1.0)
